...
test("can search Google twice", async () => {
  ...
  await driver.findElement(By.name("q")).sendKeys("apples", Key.RETURN);
  await driver.wait(until.titleIs("apples - Google Search"), 1000);
  // TODO Call .clear() on the search bar element to clear the old search term
  await driver.findElement(By.name("q")).clear();
  //await driver.wait(until.titleIs("Google Search"), 1000);
  // TODO Call .sendKeys() on the search bar element to search for a new term
  await driver.findElement(By.name("q")).sendKeys("lemons", Key.RETURN);

  // TODO Wait for the results page to load
  await driver.wait(until.titleIs("lemons - Google Search"), 10000);
});
